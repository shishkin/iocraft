...
pub trait TerminalBackend: Send {
    /// Queries the current terminal size as `(columns, rows)` outside an
    /// active render loop.
    ///
    /// Backends which cannot query a terminal without an active instance may
    /// leave the default implementation. This associated function does not
    /// affect object safety because it is only available on concrete types.
        ...
        Self: Sized,
    {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "terminal size is unavailable from this backend",
        ))
    }

    /// Writes a single canvas to a terminal outside an active render loop.
    ///
    /// This is the one-shot counterpart to [`write_canvas`](Self::write_canvas)
    /// and should include any final line ending expected by normal terminal
    /// output. Backends which only support render loops may leave the default
    /// implementation.
    fn write_once<W: io::Write>(canvas: &Canvas, writer: W) -> io::Result<()>
    where
        Self: Sized,
    {
        let _ = (canvas, writer);
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "one-shot terminal output is unavailable from this backend",
        ))
    }
    ...
    /// Enables or disables fullscreen presentation. Idempotent.
    ///
    /// ANSI backends typically implement this with the alternate screen.
    /// Backends without a fullscreen concept may leave the default no-op.
